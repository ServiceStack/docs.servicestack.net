...
In continuing with ServiceStack's [seamless integration with the ASP.NET Framework](/releases/v8_01), providing a familiar development experience that follows .NET configuration model and Entity Framework conventions
...
Previously OrmLite only supported serializing Complex Types with a [single Complex Type Serializer](/ormlite/complex-type-serializers)
...
The default Exception to this is for serialization of `object`, `List<object>` and `Dictionary<string,object>` types which is better handled by [#Script's JSON Parser](/js-utils) 
which is able to parse any valid adhoc JSON into untyped .NET generic collections, which is both mutable and able to 
[utilize C# pattern matching](/js-utils#getting-the-client_id-in-a-comfyui-output) 
...
We're excited to announce that we've ported our much loved [Background Jobs](/background-jobs)
feature for SQLite to the popular **PostgreSQL**, **SQL Server** and **MySQL** RDBMS's.

Whilst we love [SQLite + Litestream](/ormlite/litestream) for its low dev ops maintenance
allowing us to break free from 
[expensive cloud hosting hosts](/ormlite/litestream#the-right-time-for-server-side-sqlite) 
...
[mixing in](/mix-tool) the **db-identity** or **db-jobs** gist files to your host project.
...
At a minimum a command need only implement the [IAsyncCommand interface](/commands#commands-feature):
...
Thanks to ServiceStack's message-based APIs and [built-in Auto Mapping](/auto-mapping)
...
ServiceStack's flexible [API Keys feature](/auth/apikeys) now allows you to protect 
...
Using DTOs gives us all the natural [advantages of message-based APIs](/advantages-of-message-based-web-services) 
whose clean POCO models helps us [fight against complexity](/service-complexity-and-dto-roles).
...
Another major value proposition of [AI Chat](/ai-chat-api) is being able to offer a ChatGPT-like UI to your users where you're able to control the API Keys, billing, and sanctioned providers your users can access to maintain your own **Fast, Local, and Private** access to AI from within your own organization.
...
Anyone who's used ServiceStack's built-in [API Explorer](/api-explorer) or 
[Auto HTML API](/auto-html-api) UIs know that not all API Explorer UIs are created equal.

The differences are more pronounced as APIs get larger and more complex which we can see by comparing it with 
Swagger UI for rendering [AI Chat's](/ai-chat-api) `ChatCompletion` API:
