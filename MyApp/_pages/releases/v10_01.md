...
A new `[Mcp(Description = ...)]` attribute lets you provide imperative, agent-facing descriptions for DTOs exposed over MCP — such as mandatory prerequisites or confirmation steps — without polluting the regular `[Description]` used by OpenAPI generators and admin UIs:

```csharp
[Description("Submits and charges a coffee shop order.")]
[Mcp(Description =
    "Submits and charges a coffee shop order. " +
    "IMPORTANT: Before calling this you MUST first call " +
    "PreviewCoffeeShopOrder and obtain explicit customer confirmation.")]
...
When set, MCP responses (`api_search`, `api_describe`, confirmation summaries) prefer `McpDescription` over the regular `Description`. All non-MCP consumers continue to see the original `[Description]`.
