...
They [sign in with the account they already have](/chat/auth). They can talk to OpenAI, Anthropic, Google, Groq, xAI, Mistral, a local Ollama model or any other [configured provider](/chat/providers) from one model selector. They can attach documents, [dictate with their voice](/chat/media), generate images and audio, ask questions [grounded in your organization's documents](/chat/gemini-rag), [design a PDF](/chat/pdf-studio), and - most usefully - ask the assistant to **do things with [your application's own APIs](/chat/api-tools)**, with an editable approval form in front of anything consequential.
...
| Document Q&A (RAG) | Managed vector DB + indexing pipeline | [Gemini File Search extension](/chat/gemini-rag) |
...
| `gemini` | [Gemini File Search stores](/chat/gemini-rag) for RAG |
...
Authentication is also the boundary for AI Chat's state - conversation threads, [generated media](/chat/media), [Agent Profile](/chat/agents) customizations, [Projects](/chat/projects), personal [Skills](/chat/skills), [Gemini File Stores](/chat/gemini-rag), [PDF Studio](/chat/pdf-studio) workspaces and model preferences are all [scoped to the current identity](/chat/data).
