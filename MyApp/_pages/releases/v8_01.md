...
This can done by annotating Request DTOs with `[SystemJson]` attribute, specifying when to use **System.Text.Json**, 
...
This opens up the ability to use common third party tooling. A good example of this is adding OpenAPI v3 specification generation for your endpoints offered by the `Swashbuckle` package. 

:::copy
`<PackageReference Include="Swashbuckle.AspNetCore" Version="6.*" />`
:::

As a part of this release, we've also created the `ServiceStack.AspNetCore.OpenApi` package to make this integration as easy as possible, and incorporate additional information from your ServiceStack APIs into Swagger metadata.
...
Instead we'll look at how you can take advantage of the new OpenAPI v3 Swagger support using Endpoint Routing, customizing the generated specification, as well as touch on other related changes in ServiceStack v8.1.

![](/img/pages/openapi/v3/openapi-v3-swagger-ui.png)

## AppHost Initialization

To use Endpoint Routing for ServiceStack APIs, we need to change how ServiceStack is configured. Previously, the following was used to 
...
We still need to call `app.UseServiceStack` with your `AppHost`, but we also want to enable Endpoint Routing by calling `MapEndpoints()`:
...
All this setup is done for you in our updated [ServiceStack Templates](https://servicestack.net/start).
...
### ServiceStack Methods in Razor Pages

Some ServiceStack Html Helper Extension Methods in **Razor Pages** when Endpoint Routing is enabled will require access to the Request Context 
that may need to fallback to using `IHttpContextAccessor` which would need to be made available in your App with:
...
Alternatively you can upgrade to the [Pre Release](/pre-release) NuGet packages to fix the Request Context Resolution.
