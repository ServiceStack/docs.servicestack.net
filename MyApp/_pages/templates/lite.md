---
slug: templates-lite
title: Vue & React "lite" Templates
---

::: info
The "Lite" templates are obsolete, we recommend starting from [ASP.NET Core Identity Auth](/auth/identity-auth) which 
all configured with JavaScript Modules without requiring any Bundlers
:::

---

<div class="not-prose my-8 ml-20 flex ">
...
ServiceStack's [built-in bundling](/html-css-and-javascript-minification#optimal-library-bundles) 
embedded in the [dotnet tools](/dotnet-tool) which is pre-configured to use [NUglify's](https://github.com/xoofx/NUglify) 
...
Then deploy as normal, e.g. via [rsync deployments to Linux](/netcore-deploy-rsync) or to an 
[AWS EC2 container using Docker](/deploy-netcore-docker-aws-ecs).
