...
The `.ui.json` schema generates strongly typed C# models - the same source the `pdf` Startup Task writes, so what you copy from the browser matches what lands in your project.
...
public class PdfAttribute(string template) : AttributeBase
{
    ...
    public string? Namespace { get; set; } // optional per-template generated namespace
...
It runs after each development restart, keeping generated models synchronized with published template changes.
...
| `PreserveModified` | Existing files without an `<auto-generated>` marker are preserved |

The opening `<auto-generated>` comment is an explicit ownership marker. While it remains, the generator
may replace the file when its schema changes. Remove that opening marker to take ownership of a hand-tuned
model; later runs skip and report the unmarked file. An unchanged run compares the expected source with the
existing file directly; it doesn't invoke Typst or rewrite unchanged files.

To isolate generic generated names like `Item`, `From` or `Details` for one template, populate its
`PdfAttribute.Namespace`. Generation uses the override for every class from that template without changing
...
Generated source preserves the namespace override on subsequent runs.

:::info
Nothing is written in Production: `StartupTasks` only run in `DebugMode`, which maps to ASP.NET Core's `Development` environment.
:::
